...
function gamipress_leaderboard_user_position_shortcode( $atts = array() ) {
    ...
    if( ! $leaderboard_post ) {
        return gamipress_shortcode_error( sprintf( __( 'Leaderboard with ID %d not found.', 'gamipress-leaderboards' ), $leaderboard_id ), 'gamipress_leaderboard_user_position' );
    }

    // Return if not is a leaderboard
    if( $leaderboard_post->post_type !== 'gp_leaderboard' ) {
        return gamipress_shortcode_error( sprintf( __( 'Leaderboard with ID %d is not a leaderboard.', 'gamipress-leaderboards' ), $leaderboard_id ), 'gamipress_leaderboard_user_position' );
    }

    // Return if leaderboard was not published
    if( $leaderboard_post->post_status !== 'publish' ) {
        return gamipress_shortcode_error( sprintf( __( 'Leaderboard with ID %d is not published.', 'gamipress-leaderboards' ), $leaderboard_id ), 'gamipress_leaderboard_user_position' );
    }
    ...
    $atts = shortcode_atts( array(
        ...
        'current_user'      => 'yes',
        'user_id'           => '0',
        'text'              => __( '{user}, you are in position {position} on {leaderboard_link}', 'gamipress-leaderboards' ),
        'not_ranked_text'   => __( '{user}, you are not ranked on {leaderboard_link}', 'gamipress-leaderboards' ),
