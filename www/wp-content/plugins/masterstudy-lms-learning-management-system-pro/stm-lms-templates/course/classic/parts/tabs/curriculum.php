...
$lesson_type  = get_post_meta( $post_id, 'type', true );
...
if ( ! empty( $curriculum ) ) :
	...
	<div class="stm-curriculum">
		...
		<?php foreach ( $curriculum as $curriculum_item ) : ?>
			...
			$meta           = '';
			$icon           = 'stmlms-text';
			$hint           = esc_html__( 'Text Lesson', 'masterstudy-lms-learning-management-system-pro' );
			...
			if ( 'stm-quizzes' === $content_type ) {
				...
				$icon = 'stmlms-quiz';
				$hint = esc_html__( 'Quiz', 'masterstudy-lms-learning-management-system-pro' );
				if ( ! empty( $q ) ) :
					$meta = sprintf(
						/* translators: %s: questions number */
						_n(
							'%s question',
							'%s questions',
							count( explode( ',', $q ) ),
				...
				if ( 'slide' === $type ) {
					...
					$hint = esc_html__( 'Slides', 'masterstudy-lms-learning-management-system-pro' );
				}

				if ( 'video' === $type ) {
					$icon = 'stmlms-slides';
					$hint = esc_html__( 'Video', 'masterstudy-lms-learning-management-system-pro' );
				}
				if ( 'stream' === $type ) {
					$icon = 'fab fa-youtube';
					$hint = esc_html__( 'Live Stream', 'masterstudy-lms-learning-management-system-pro' );
				}
				if ( 'zoom_conference' === $type ) {
					$icon = 'fas fa-video';
					$hint = esc_html__( 'Zoom meeting', 'masterstudy-lms-learning-management-system-pro' );
				}
				if ( ! empty( $meta ) ) {
