...
if ( ! class_exists( 'ReduxFramework_extension_import_export' ) ) {
	...
	class ReduxFramework_extension_import_export {
		...
		public function link_options() {
			if ( ! isset( $_GET['secret'] ) || md5( md5( Redux_Helpers::get_auth_key_secret_key() ) . '-' . $this->parent->args['opt_name'] ) != $_GET['secret'] ) {
		...
		public function download_options() {
			...
			if ( isset( $_GET['action'] ) && 'redux_download_options-' . $this->parent->args['opt_name'] == $_GET['action'] ) {
		...
		public function remove_cookie() {
			...
			if ( 'import_export_default' == $_COOKIE['redux_current_tab'] ) {
