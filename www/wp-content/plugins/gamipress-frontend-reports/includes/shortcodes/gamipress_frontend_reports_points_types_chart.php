<?php
/**
 * GamiPress Frontend Reports Points Types Chart Shortcode
 *
 * @package     GamiPress\Frontend_Reports\Shortcodes\Shortcode\GamiPress_Frontend_Reports_Points_Types_Chart
 * @since       1.0.0
 */
...
/**
 * Register the [gamipress_frontend_reports_points_types_chart] shortcode.
 *
 * @since 1.0.0
 */
function gamipress_register_frontend_reports_points_types_chart_shortcode() {

    gamipress_register_shortcode( 'gamipress_frontend_reports_points_types_chart', array(
        'name'              => __( 'Points Types Chart', 'gamipress-frontend-reports' ),
        'description'       => __( 'Render a chart with user points balances of desired points type(s).', 'gamipress-frontend-reports' ),
        'icon'              => 'star-filled',
        'group'             => 'frontend_reports',
        'output_callback'   => 'gamipress_frontend_reports_points_types_chart_shortcode',
        'tabs' => array(
            'general' => array(
        ...
        'fields'            => array(
            'type' => array(
                'name'          => __( 'Points Type(s)', 'gamipress-frontend-reports' ),
                'desc'          => __( 'Choose the points type(s).', 'gamipress-frontend-reports' ),
            ...
            'amount' => array(
                ...
                'desc'          => __( 'Choose the amount type to use as chart values.', 'gamipress-frontend-reports' ),
                'type'          => 'select',
                'options'       => array(
...
add_action( 'init', 'gamipress_register_frontend_reports_points_types_chart_shortcode' );
 ...
 */
function gamipress_frontend_reports_points_types_chart_shortcode( $atts = array(), $content = '' ) {

    global $gamipress_frontend_reports_template_args;
    $shortcode = 'gamipress_frontend_reports_points_types_chart';
    ...
    if( $is_single_type ) {
        ...
        if( $all_types_wrong )
            return gamipress_shortcode_error( __( 'All types provided aren\'t valid registered points types.', 'gamipress-frontend-reports' ), $shortcode );
    ...
    foreach( $types as $type ) {
        ...
        switch( $atts['amount'] ) {
            case 'deducted':
                $value = gamipress_get_user_points_deducted( $atts['user_id'], $type );
                break;
            case 'expended':
                $value = gamipress_get_user_points_expended( $atts['user_id'], $type );
                break;
            case 'earned':
            default:
                $value = gamipress_get_user_points( $atts['user_id'], $type );
                break;
        }
    ...
    switch( $atts['amount'] ) {
        case 'deducted':
            $gamipress_frontend_reports_template_args['label'] = __( 'Deducted', 'gamipress-frontend-reports' );
            break;
        case 'expended':
            $gamipress_frontend_reports_template_args['label'] = __( 'Expended', 'gamipress-frontend-reports' );
            break;
        case 'earned':
        default:
    ...
    // Bail if no stats
    if( empty( $stats ) )
        return gamipress_shortcode_error( __( 'Any type provided isn\'t a valid registered points type.', 'gamipress-frontend-reports' ), $shortcode );
    ...
    if( ! gamipress_frontend_reports_stats_meets_chart_style_minimum( $stats, $atts['style'] ) )
        return gamipress_shortcode_error( __( 'You need to provide more points types for this chart style.', 'gamipress-frontend-reports' ), $shortcode );
     ...
     * @param string    $content    Shortcode content
     */
    return apply_filters( 'gamipress_frontend_reports_points_types_chart_shortcode_output', $output, $atts, $content );
