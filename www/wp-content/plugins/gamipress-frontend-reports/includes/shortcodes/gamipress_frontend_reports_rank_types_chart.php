<?php
/**
 * GamiPress Frontend Reports Rank Types Chart Shortcode
 *
 * @package     GamiPress\Frontend_Reports\Shortcodes\Shortcode\GamiPress_Frontend_Reports_Rank_Types_Chart
 * @since       1.0.0
 */
...
/**
 * Register the [gamipress_frontend_reports_rank_types_chart] shortcode.
 *
 * @since 1.0.0
 */
function gamipress_register_frontend_reports_rank_types_chart_shortcode() {

    gamipress_register_shortcode( 'gamipress_frontend_reports_rank_types_chart', array(
        'name'              => __( 'Rank Types Chart', 'gamipress-frontend-reports' ),
        'description'       => __( 'Render a chart with user ranks of desired rank type(s).', 'gamipress-frontend-reports' ),
        'icon'              => 'rank',
        'group'             => 'frontend_reports',
        'output_callback'   => 'gamipress_frontend_reports_rank_types_chart_shortcode',
        'tabs' => array(
            'general' => array(
        ...
        'fields'            => array(
            'type' => array(
                'name'          => __( 'Rank Type(s)', 'gamipress-frontend-reports' ),
                'desc'          => __( 'Choose the rank type(s).', 'gamipress-frontend-reports' ),
            ...
            'amount' => array(
                ...
                'options'       => array(
                    ...
                    'priority'  => __( 'Priority', 'gamipress-frontend-reports' ),
                ),
                'default'       => 'earned'
            ),
            'current_user' => array(
                'name'          => __( 'Current User', 'gamipress-frontend-reports' ),
                'description'   => __( 'Show ranks of the current logged in user.', 'gamipress-frontend-reports' ),
                'type' 		    => 'checkbox',
            ...
            'user_id' => array(
                ...
                'description'   => __( 'Show ranks of a specific user.', 'gamipress-frontend-reports' ),
                'type'          => 'select',
                'default'       => '',
...
add_action( 'init', 'gamipress_register_frontend_reports_rank_types_chart_shortcode' );
 ...
 */
function gamipress_frontend_reports_rank_types_chart_shortcode( $atts = array(), $content = '' ) {

    global $gamipress_frontend_reports_template_args;
    $shortcode = 'gamipress_frontend_reports_rank_types_chart';
    ...
    if( $is_single_type ) {
        ...
        if( $all_types_wrong )
            return gamipress_shortcode_error( __( 'All types provided aren\'t valid registered rank types.', 'gamipress-frontend-reports' ), $shortcode );
    ...
    foreach( $types as $type ) {
        ...
        switch( $atts['amount'] ) {
            case 'priority':
                $rank = gamipress_get_user_rank( $atts['user_id'], $type );
                $value = ( $rank ? absint( $rank->menu_order ) : 0 );
                break;
            case 'earned':
            default:
    ...
    switch( $atts['amount'] ) {
        case 'priority':
            $gamipress_frontend_reports_template_args['label'] = __( 'Priority', 'gamipress-frontend-reports' );
            break;
        case 'earned':
        default:
    ...
    // Bail if no stats
    if( empty( $stats ) )
        return gamipress_shortcode_error( __( 'Any type provided isn\'t a valid registered rank type.', 'gamipress-frontend-reports' ), $shortcode );
    ...
    if( ! gamipress_frontend_reports_stats_meets_chart_style_minimum( $stats, $atts['style'] ) )
        return gamipress_shortcode_error( __( 'You need to provide more rank types for this chart style.', 'gamipress-frontend-reports' ), $shortcode );
     ...
     * @param string    $content    Shortcode content
     */
    return apply_filters( 'gamipress_frontend_reports_rank_types_chart_shortcode_output', $output, $atts, $content );
