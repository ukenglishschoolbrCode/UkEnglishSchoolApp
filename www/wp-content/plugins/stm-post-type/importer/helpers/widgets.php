...
function stm_theme_import_widgets( $layout ) {
	...
	if ( ! isset( $exclude_layouts[ $layout ] ) ) {

		$widgets_shop = STM_POST_TYPE_PATH . '/importer/demos/products/widgets/widget_data_shop.json';

		if ( file_exists( $widgets_shop ) ) {
			$encode_widgets_array = $wp_filesystem->get_contents( $widgets_shop );
			stm_import_widgets( $encode_widgets_array );
		}
	}
...
function stm_get_new_widget_name( $widget_name, $widget_index ) {
	...
	if ( $current_sidebars && is_array( $current_sidebars ) ) {
