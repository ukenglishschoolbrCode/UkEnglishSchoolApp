...
if ( ! class_exists( 'ReduxFramework_media' ) ) {
	...
	class ReduxFramework_media {
		...
		public function render() {
			...
			if ( isset( $this->field['mode'] ) && false == $this->field['mode'] ) {
				$this->field['mode'] = 0;
			}
			...
			if ( ( ! empty( $this->field['url'] ) && true === $this->field['url'] ) || isset( $this->field['preview'] ) && false === $this->field['preview'] ) {
				$hide = '';
			}
			...
			if ( isset( $this->field['readonly'] ) && false === $this->field['readonly'] ) {
				$readOnly = '';
			}
